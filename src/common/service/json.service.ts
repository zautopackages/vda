...
import { JSON_CORRECTOR_PROMPT } from "src/agents/prompts/json-corrector.prompt";
...
export class JsonService{
    ...
    async correctJson(input:string){
        ...
        try{
            const { content } = await this.llmService.generate({prompt:dynamicPrompt,model: LLM_MODELS.bedrock.ANTHROPIC_CLAUDE_3_SONNET});
